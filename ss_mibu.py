...
def main(user, passwd, step):
    ...
    if step == '':
        print("已设置为随机步数（11000-25000）")
        step = str(random.randint(11000, 25000))
    login_token = 0
...
def start():
    ...
    user = os.environ.get("miuser")
    # 登录密码（用#隔开，例如123456#123456#123456）
    passwd = os.environ.get("mipass")
    ...
    if len(user_list) == len(passwd_list):
        ...
        try:
            push_tg(os.environ.get("TG_BOT_TOKEN"), os.environ.get("TG_CHAT_ID"), push)
        except:
            print('推送出错')
